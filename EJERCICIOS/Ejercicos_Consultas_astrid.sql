...
/* mostrar todos los clientes (id, nombre_completo, fecha_nacimiento, now()-fecha_nacimiento AS edad)
que se apelliden 'Lasa' */
SELECT id, nombre_completo, fecha_nacimiento, now()-fecha_nacimiento AS edad
FROM cliente
WHERE nombre_completo LIKE '%Lasa%';
...
SELECT canal_venta
FROM venta
WHERE canal_venta NOT LIKE 'Online' AND canal_venta NOT LIKE 'Offline';
...
SELECT id, proveedor, contacto, email, telefono, saldo, fecha_ult_compra
FROM proveedores
WHERE saldo;

/*Todas las ventas que el precio unitario se mayor al avg*/
SELECT precio_unitario
FROM venta
WHERE precio_unitario > (SELECT avg(precio_unitario) FROM venta);

/* Ejercicio 4: localiza la venta Online que ha obtenido mayor beneficio */
SELECT *
FROM venta
WHERE canal_venta LIKE 'Online' 
ORDER BY importe_venta DESC LIMIT 1;

SELECT *
FROM venta;
 
/* Ejercicio 5: quiero saber cuantos tipos de producto distintos se han vendido */
SELECT COUNT(DISTINCT (tipo_producto)) as tipos_Distintos_Productos
FROM venta;
...
SELECT pais, tipo_producto, count(*), max(precio_unitario)
FROM venta
...
FROM venta
GROUP BY pais LIMIT 1
) ;

SELECT pais, max(precio_unitario)
FROM venta
GROUP BY pais;
...
SELECT pais, tipo_producto, DATEDIFF(fecha_envÍo, fecha_pedido) as 'Tiempo media de entrega'
FROM venta
WHERE zona LIKE 'Europa'
ORDER BY pais, DATEDIFF(fecha_envÍo, fecha_pedido) DESC;
...
FROM venta
WHERE pais like '%Republic%' AND zona IN (select zona
										from venta
										group by zona
...
SELECT sum(unidades) as 'numero total de unidades', zona
FROM venta
group by zona
having sum(unidades) < (select sum(unidades) * 0.10 as '10% total'
						from venta) ;
...
SELECT pais, DATEDIFF(fecha_envÍo, fecha_pedido) as Tiempo_media_de_entrega, min(importe_venta)
FROM venta
GROUP BY  pais
HAVING  Tiempo_media_de_entrega > 10 AND  min(importe_venta) > (select avg(importe_venta)
																from venta);
    
/* Ejercicio 14: paises que el importe 
mínimo de venta del país sea mayor que el 
más bajo de los máximos por zona */
...
having min(importe_venta) > (select max(importe_venta)
							from venta
                            group by zona
                            order by max(importe_venta) ASC
                            limit 1);
                                
							
/* Ejercicio 15: ventas Online con fecha_pedido anterior a 2022 y con beneficio superior a la media */
SELECT  id_pedido, fecha_pedido, canal_venta, (importe_venta - importe_coste) as beneficio
FROM venta
WHERE canal_venta LIKE 'Online' AND year(fecha_pedido) < 2022 AND
(importe_venta - importe_coste)  > (SELECT AVG(importe_venta - importe_coste)
									FROM venta);
					
/* Ejercicio 16: ventas en Europa de países que tengan una total de ventas (suma importe_venta) mayor
de un millón */
SELECT pais, sum(importe_venta), zona
FROM venta
group by pais
having zona LIKE 'Europa' AND (SELECT sum(importe_venta) > 1000000
								FROM venta) ;

								
/* ejercicio 17: quiero saber los paises (pais, suma total de importes de venta) donde el precio unitario 
mínimo es mayor que el precio unitario medio para todo el mundo y 
que estén en una zona donde la media de importe_venta de la zona sea mayor que la media mundial de importe_venta */
SELECT pais, sum(importe_venta), zona, min(precio_unitario)
FROM venta
group by pais, zona
having min(precio_unitario) > (SELECT AVG(precio_unitario)
								FROM venta)
AND (SELECT AVG(importe_venta)
	FROM venta
    group by zona
    
     ) > (SELECT AVG(importe_venta)
						FROM venta)
;
/***************************************************************/
select pais, sum(importe_venta)
...
/* Ejercicio 21: saldo de todos los proveedores que hayan hecho ventas en el pais Tajikistan  */

/* Ejercicio 22: muestra un listado de clientes (id, nombre_completo, .... muestra todos los atributos)
que han realizado al menos 2 compras Online  */

/* Ejercicio 23: listado de proveedores que han realizado ventas en el pais donde más clientes han comprado */




/***************************************************************************************************************/
/*EJERCICIOS RESULTOS POR EL PROFESOR*/
use ejemplo_ventas;
